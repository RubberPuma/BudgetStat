...
class UsersTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.limi1 = LimitFactory(user=user1)
        self.limi2 = LimitFactory(user=user1)
        self.expense1 = ExpenseFactory(user=user1)
        self.expense2 = ExpenseFactory(user=user1)

    def test_get_all_user_limits(self):
        # Given
        url = reverse("user-limits", kwargs={"pk": self.user1.pk})
        limits = user1.limit_set.all()
    ...
    def test_get_all_user_expenses(self):
        ...
        expenses = user1.expense_set.all()
        serializer = LimitSerializer(expenses, many=True)
